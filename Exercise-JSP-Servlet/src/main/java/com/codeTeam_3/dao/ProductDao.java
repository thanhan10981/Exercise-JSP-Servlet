...
public class ProductDao {
    ...
    public List<ProductView> searchByName(String lang, String name, int limit) {
        String sql = """
            SELECT p.ProductID, p.Price, p.Weight,
                   pt.ProductName, pt.ProductDescription,
                   pct.CategoryName
            FROM Product p
            JOIN ProductTranslation pt ON p.ProductID = pt.ProductID AND pt.LanguageID = ?
            JOIN ProductCategoryTranslation pct ON p.ProductCategoryID = pct.ProductCategoryID AND pct.LanguageID = ?
            WHERE pt.ProductName LIKE ?
            LIMIT ?
            """;

        List<ProductView> list = new ArrayList<>();
        try (java.sql.Connection c = dbMysql.getConnection();
